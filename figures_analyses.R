...
tableau_diete
#Mettre dans latex
writeLines(kable(tableau_diete, format = 'latex'),con = 'tableau_diete.tex')
...
#Créer graphique
#ouvrir pdf
pdf("graph_reseau.pdf",width = 6,height = 6) 
# 2. créer graph
...
#ouvrir pdf
pdf("graph_reseau_classe.pdf",width = 6,height = 6) 
# 2. créer graph
...
summary(zzz)
zzz<-lm(liens_region0$Freq~liens_region0$meme_region)
...
# 2. créer graph
plot(jitter(liens_region0$meme_region),liens_region0$Freq,xlab = 'Région de naissance identique ou pas',ylab = 'Fréquence des liens')
legend('topright',legend = 'P-value=0,0495')
ablineclip(zzz,x1=0,x2=1)
# fermer pdf
dev.off()
